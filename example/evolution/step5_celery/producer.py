"""
Step 5 的派工層: 與 Step 4 的 producer.py 相比, 只有一行不同。

    Step 4: task.crawl(**job)          直接呼叫函式, 當場執行, 執行完才輪到下一顆
    Step 5: task.crawl.delay(**job)    把任務送進 RabbitMQ, 送完立刻返回, 由 worker 執行
...
執行前提:
    1. RabbitMQ 已啟動。
    2. worker 已啟動: uv run celery -A example.evolution.step5_celery.worker worker --loglevel=info
"""
from example.evolution.step5_celery import task
from example.evolution.step5_celery.config import END_DATE, START_DATE, STOCK_IDS


def build_jobs():
    """產生這一輪的任務清單, 內容與 Step 4 相同。"""
    return [
