...
上面幾發 curl 已經蒐集到三種狀態碼了。狀態碼是 HTTP 的通用語言，第一位數字就分好了陣營——先看全景，再對回本篇：
...
| | | **304** Not Modified | 資源沒變，用你快取的就好（瀏覽器收到這個就不重新下載）|
| **4xx** | **呼叫方的錯** | **400** Bad Request | 請求格式有問題（JSON 壞掉之類）|
...
| | | **502** Bad Gateway | 反向代理（Nginx）打後端但後端沒回應 |
| | | **503** Service Unavailable | 服務暫時不可用（部署中、過載）|

記法：**2xx 成功；3xx 去別的地方；4xx 呼叫方的錯**（改請求再試）；**5xx 提供方的錯**（呼叫方等修就好）。

本篇實際遇到的三個對照：**200**（正常查詢）、**404**（我們設計的——業務邏輯：查無此股票）、**422**（框架送的——機械規則：參數不合法）。好的 API 讓兩層錯誤各司其職：404 是你寫的 `raise HTTPException`，422 是 FastAPI 從型別註記自動幫你擋的。
...
- **三個出口的分工，現在完整了**：Metabase 給人看（第 8 章）、BigQuery 給大規模分析（第 15 章）、API 給程式呼叫（本篇）。三者都只是「讀取 MySQL 的不同姿勢」——上游的爬蟲 pipeline 一行都不用改。這就是分層架構的威力。
