...
2. 說得出哪些元件該畫進圖裡、哪些不該畫。
3. 把圖存成可再編輯的 `.drawio` 檔放進 repo，並用 VS Code 直接開啟。
4. 啟用 GCP 圖形庫，用官方圖示畫雲端架構。
5. 分辨 draw.io 與 Mermaid 各自適合的場合。
...
| `mongodb`、`mongo-express`（補充F） | 不畫 | 跟主線資料流無關 |
...
| 格式 | 是什麼 | 什麼時候用 |
|---|---|---|
| `.drawio` | 原始檔，內容是 XML | 要再修改、要進 git、要跟別人協作 |
| `.png` / `.svg` | 出圖 | 貼進投影片、文件、簡報 |
...
| 檔案 | 說明 |
|---|---|
| `課程手冊/drawio/本地端架構圖.drawio` | 本地端完成版，畫完可以打開對照 |
| `課程手冊/drawio/GCP雲端架構圖.drawio` | 雲端版範本 |
...
第一次進入會問檔案要存在哪裡。選「裝置」（Device），檔案就留在自己電腦上。

**A-2 認識介面三區**

![draw.io 介面三區](images/chH/03-drawio介面三區.jpg)
...
`檔案 → 儲存`，選擇存到裝置，命名為 `本地端架構圖.drawio`。之後每改一段就存一次。
...
改好一個框的樣式後，選取它按 `Cmd+C`，再選取另一個框，用 `編輯 → 貼上樣式`——只套用外觀，不動文字。

> **原則二：旁支不要搶主幹的視覺重量。** 灰色加虛線，一眼就分得出「這條線是人在看，不是資料在流」。
...
![本地端架構圖](images/chH/01-本地端架構圖.jpg)
...
![匯出為選單](images/chH/06-匯出為選單.jpg)

對話框裡三個選項要知道：

| 選項 | 建議 | 理由 |
|---|---|---|
| 縮放 | 貼投影片用 200% | 100% 投影出來會糊 |
| 透明背景 | 貼深色投影片時勾 | 否則圖片會有一塊白底 |
| 包含圖表副本 | 勾 | 把 XML 藏進 PNG，之後可以把 PNG 拖回 draw.io 繼續改 |
...
![啟用 GCP 圖形庫](images/chH/04-啟用GCP圖形庫.jpg)
...
![GCP 雲端架構圖](images/chH/02-GCP雲端架構圖.jpg)
...
VM1 和 VM2 各用一個虛線框把裡面的服務圈起來，框的標題寫 VM 名稱。這樣一眼看得出「哪些服務跑在同一台機器上」——這正是第 16 章拆成兩台之後最需要表達的資訊。
...
|---|---|---|
| 怎麼畫 | 滑鼠拖曳 | 寫文字語法 |
| 版面控制 | 自己決定每個框的位置 | 由排版引擎決定，只能給提示 |
| 放在哪 | 圖檔 + 原始檔兩個檔案 | 直接寫在 `.md` 檔裡 |
| 改一個字 | 開編輯器改完再匯出一次 | 改文字存檔即可 |
| 適合 | 版面要精緻的圖，例如投影片、對外文件 | 跟著程式碼走的圖，例如 README、設計文件 |
...
```mermaid
flowchart LR
    U(("使用者")) -->|HTTPS| CR["Cloud Run<br/>stock-api（補充G 選讀）"]
    CR -.->|查詢| SQL
...
用兩張並排的圖表達「搬家前」與「搬家後」：搬家前是單台 VM 跑全部服務，搬家後是 VM1／VM2 加 Cloud SQL。重點在讓讀者一眼看出哪些東西移動了。
...
- 先畫資料流主幹，再加旁支，最後疊編排層。一層一個抽象高度。
- 線條樣式的規則自己定，但要一致，而且要寫進圖例。
- 外部系統與可替換方案用虛線框標示，讀者才知道哪些東西不歸自己管。
- `.drawio` 原始檔要進 git，PNG 只是出圖。匯出時勾「包含圖表副本」，PNG 也能拖回去繼續編輯。
- 雲端圖用官方圖示庫，圖庫沒有的服務就用註記說明，不要拿相近的圖示硬湊。
- 圖會跟著程式碼一起改的用 Mermaid，要拿去簡報的用 draw.io；draw.io 可以匯入 Mermaid 語法，兩條路能接起來。
