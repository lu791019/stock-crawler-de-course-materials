...
> 這一章你不會寫任何真實邏輯，只用一個「假任務」把 Celery 的骨架完整跑一遍。目標是讓你親手感受「非同步派送」到底是什麼。這是整套系統的地基，請務必自己動手跑成功再往下走。
...
這張圖就是這一章要讓你「親眼看到」的東西。待會實作時你會自己驗證它。
...
靠的就是環境變數覆蓋，程式碼不用改。第 3 章把 worker 搬進 Docker 時你會親手用到。
...
這就對應到前面那張「非同步時間軸」圖。你剛剛親手把它驗證出來了。
...
**練習 3（重點）：親手對照 concurrency 對「順序」的影響**
