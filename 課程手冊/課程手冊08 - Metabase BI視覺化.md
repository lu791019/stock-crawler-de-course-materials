...
- **右下「下載」（雲朵/箭頭圖示）**：把結果匯出 CSV / Excel / JSON——老闆要原始數字時直接給
...
5. **加文字卡片**：上方 **T 圖示**——放個標題「台股每日戰情」或說明文字，Dashboard 更像正式報表。
6. **分頁標籤**：Dashboard 上方「＋新增分頁」——圖太多時分成「總覽」「個股」兩頁。
7. 右上「儲存」離開編輯模式。
8. **定期刷新**：Dashboard 檢視模式右上**時鐘圖示** → 選自動刷新間隔（如 15 分鐘）——投影在牆上的戰情室就是這樣做的。

> ✅ 一個頁面同時看到走勢、成交量、最新價，還能用篩選器互動——這就是給老闆看的那一頁。
>
> 💡 **公開分享**：管理員 → 設定 → 公開分享 開啟後，Question/Dashboard 的分享圖示能產生「不用登入就能看」的公開連結。方便，但等於把資料開大門——正式環境要想清楚再開。（按鈕名稱以 Metabase v0.62 為準，新版可能微調。）
