...
- **並發（concurrency）**：交錯處理很多件事，**不一定真的同時**。像一個人同時煮飯又洗衣——他其實是在兩件事之間切換，趁飯在燉的空檔去洗衣。
- **並行（parallelism）**：**真的同時**執行。像兩個人，一個煮飯、一個洗衣，同一秒都在動。
...
| pool | 底層是什麼 | 併發模型 | concurrency 合理範圍 | 適合 | 注意 |
|------|-----------|---------|:------------------:|------|------|
| `prefork`（預設）| 多個子行程（fork） | 並行（繞過 GIL）| ≈ CPU 核心數 | CPU 密集 | 吃記憶體、啟動慢；容器內要留意 RAM 上限 |
| `threads` | 作業系統執行緒 | 並發（受 GIL）| 數十 | I/O 密集 | 標準庫就有、不用裝套件，最無腦的 I/O 選項 |
| `gevent` | 協程（綠色執行緒）| 並發（受 GIL）| 數百 | I/O 密集（**爬蟲首選**）| 要先 `uv add gevent`；靠 monkey-patch 把阻塞 I/O 換成非阻塞，極少數 C 擴充套件不相容 |
| `eventlet` | 協程（gevent 的同類替代品）| 並發（受 GIL）| 數百 | I/O 密集 | 與 gevent 同定位、二選一即可；近年維護趨緩，新專案建議選 gevent |
| `solo` | 單行程單線，任務在主行程裡直接跑 | 無併發 | 1 | 除錯、教學 | 一次只跑一個任務、log 最乾淨；查單一任務的問題時很好用 |
...
| 概念 | 一句話 | 並發還是並行 | repo 例子 | 適合 |
|------|--------|-------------|-----------|------|
| 同步 | 呼叫後等它做完才往下 | — | `crawler_finmind(...)` | 簡單、有先後依賴 |
| 非同步 | 丟出去就繼續，不等 | — | `.delay(...)` / `.apply_async(...)` | 不想被耗時工作卡住 |
| 多執行緒 | 一行程多條線、共享記憶體 | 偏並發（受 GIL）| `--pool=threads/gevent` | I/O 密集 |
| 多行程 | 多個獨立行程、各用一核 | 並行 | `--pool=prefork`（預設）| CPU 密集 |
| 分散式 | 多台機器靠 broker 協調 | 並行 | 多個 worker 連同一 RabbitMQ | 量太大、要水平擴充 |
