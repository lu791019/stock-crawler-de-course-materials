...
7. 說得出部署後的 API 層為什麼是一個微服務，以及「一個元件微服務化」跟「系統微服務架構」的差別

## 先搞懂

### 對外服務要解的三個問題

第 14 章的「想一想」問過：VM 停機後外部 IP 會換，把網址發給別人用是什麼問題？現在正面回答。把服務開給外面的人用，馬上遇到三件事：
...
| 沒流量 | **自動縮到 0 個容器，這段時間不計費**——直接滿足本課程「下課要停機」的需求，而且不用自己動手 |
...
### 什麼工作適合 Cloud Run，什麼不適合

課程裡只有 API 交給 Cloud Run，爬蟲 worker 和 Airflow 都還留在 VM 上。這不是因為做不到，而是**工作的形狀不同**：

| | API（適合 Cloud Run） | Airflow scheduler（不適合） |
|---|---------------------|--------------------------|
| 什麼時候要做事 | 有人呼叫才做 | 一直醒著，每分鐘檢查有沒有到期的排程 |
| 沒事做的時候 | 縮到零，不計費 | 縮到零就不排程了 |
| 單次執行多久 | 幾百毫秒 | 一個 task 可能跑幾十分鐘 |
| 有沒有狀態 | 沒有，每個請求各自獨立 | 有，要持久化 logs 與執行紀錄 |

判斷的原則：**「有人呼叫才做事、做完就結束、不用記住上次做了什麼」的工作適合 Cloud Run**。API 完全符合，所以放上去很自然。

Airflow 的 scheduler 每一項都相反，它需要一直執行。Cloud Run 有 `--min-instances=1` 可以讓容器常駐，但那等於放棄了 Cloud Run 的所有優點（自動縮零、按量計費），成本還可能比一台 VM 高——這種時候就該用 VM 自架，或用專門的託管排程服務 Composer（第 17 章會比較）。

爬蟲 worker 的情況類似：它要一直連著 RabbitMQ 等任務進來，屬於常駐型，所以留在 VM2 上。

### 這章做完，API 層就是一個微服務

部署上 Cloud Run 之後，stock-api 具備微服務的全部特徵：

| 微服務的特徵 | stock-api 在本章的對應 |
|-------------|----------------------|
| 獨立容器、獨立部署 | 自己的 image、自己的 deploy 指令，改版不動系統其他部分 |
| 無狀態 | 每個請求各自獨立，狀態全部在資料庫裡 |
| 自動擴縮 | 流量大自動多開容器、沒流量縮到零 |
| 版本化交付 | image v1／v2＋revision，換版零停機、回滾一條指令 |
| 透過網路介面對外 | 固定 HTTPS 網址，呼叫方只認網址不認機器 |

範圍要說清楚：**微服務化的是 API 這一層，也只有這一層**——它是課程系統裡第一個、也是唯一一個微服務化的零件。worker、Airflow、RabbitMQ 都留在 VM 上，資料庫是全系統共用的一顆。整個系統的準確定位是「分散式任務處理系統＋一個微服務化的 API 元件」，不是微服務架構。服務邊界怎麼拆、拆完資料怎麼分，屬於微服務架構設計的範疇，本課程不涵蓋。
