...
| 站 | 章節 | 教什麼 | 當章用在哪裡 |
|---|------|--------|-------------|
| 1 | 本章 Part D＋Part J | 服務帳戶是給程式用的身分；先建身分、暫不授權（理由見 Part D）；Part J 是團體專案上雲的完整規劃——IAM 授權、防火牆、密碼與 .env | 幫第 15 章把憑證發好；讓全組共用同一台 VM |
...
### Part J：團體專案上雲——IAM、防火牆、密碼與 .env 的完整規劃

系統已經上雲、防火牆也開好了，最後一個場景是**多人**：期末專題分組後，要把**你們自己的系統**搬上雲，全組一起操作。部署模型跟本章一樣——**一台 VM 全組共用**，Part F 到 Part I 的每一步照做，差別在於多人環境要多設定五層東西。本段以課程系統當範例示範每一層；換成你們的專案時，對應到相同的位置即可。

先看整體規劃，這張表就是團體專案上雲的檢查清單，J-1 到 J-6 逐層展開：

| 層 | 要設定什麼 | 誰做 | 段落 |
|----|-----------|------|------|
| GCP 專案 | 誰開專案（＝帳單算誰的）、預算警告 | 一位組員（下稱開專案者） | J-1 |
| IAM | 組員加進專案，給兩個角色 | 開專案者 | J-2 |
| 機器 | 組員 SSH 帳號、docker 群組 | 組員連線＋開專案者開通 | J-3、J-4 |
| 網路 | 防火牆來源加全組 IP | 開專案者 | J-5 |
| 機密 | 換強密碼、.env 的存放與傳遞 | 全組約定 | J-6 |
...
課程系統一路用 `root/1234`，前提是 Part I 的防線——3306 不對公網開、Web 介面只放白名單 IP。這個前提在團體專案**依然成立，但不再足夠**：白名單裡的 IP 從一個變成一組，而且家用 IP 會被 ISP 回收再發給別人。人越多、開機時間越長，弱密碼殘存的風險就越大。**團體專案上雲前，把資料庫密碼換成自己的強密碼**，做法有三步：
...
# ③ 重建 MySQL 容器讓新密碼生效（volume 裡的舊資料若已用舊密碼初始化，要先清掉 volume 再起）
sudo docker compose -f docker-compose-local.yml up -d --force-recreate mysql
```
...
- 加組員不需要共用任何密碼或金鑰——每個人用自己的 Google 帳號登入，這正是 IAM 身分制的意義；資料庫密碼這類機密不歸 IAM 管，J-6 用 .env 處理，第 16 章再交給 Secret Manager
