...
4. 左側點「**OS 和儲存空間**」→「變更」：作業系統選 Ubuntu、版本 24.04 LTS（= `--image-family`），磁碟大小填 20GB（= `--boot-disk-size`）。（截圖待補）
...
- 路徑：「≡」選單 →「**虛擬私有雲網路（VPC network）**」→「**防火牆**」。清單裡看得到 CLI 建立的 `allow-stock-web` 規則，點進去可以改 port、來源 IP 範圍、目標標籤，每個欄位對應 `firewall-rules create` 的一個參數。（截圖待補）

### 對照收工：用 Console 停機／開機

- 回到「VM 執行個體」清單，勾選 VM 後上方出現「**停止**」「**啟動／繼續**」按鈕；或點每列最右邊的「⋮」選單操作。效果跟 `instances stop` / `start` 完全相同，停止前會跳出確認視窗。（截圖待補）
