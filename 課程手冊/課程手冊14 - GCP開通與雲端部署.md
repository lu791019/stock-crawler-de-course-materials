...
上雲之後，每一個操作——開 VM、寫 BigQuery、讀密碼——都要先通過權限檢查。負責這件事的系統叫 **IAM**（Identity and Access Management，身分與存取管理），它回答的問題只有一句：**誰（身分）可以對哪個資源做什麼（角色與權限）**。
...
| 站 | 章節 | 教什麼 | 當章用在哪裡 |
|---|------|--------|-------------|
| 1 | 本章 Part D | 服務帳戶是給程式用的身分；建立時刻意不給任何角色 | 幫第 15 章把憑證發好 |
