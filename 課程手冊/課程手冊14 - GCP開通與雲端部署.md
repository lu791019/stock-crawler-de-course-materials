...
先看整體規劃，這張表就是團體專案上雲的檢查清單，步驟 1 到 步驟 6 逐層展開：

| 層 | 要設定什麼 | 誰做 | 段落 |
|----|-----------|------|------|
| GCP 專案 | 誰開專案（＝帳單算誰的）、預算警告 | 一位組員（下稱開專案者） | 步驟 1 |
| IAM | 組員加進專案，給兩個角色 | 開專案者 | 步驟 2 |
| 機器 | 組員 SSH 帳號、docker 群組 | 組員連線＋開專案者開通 | 步驟 3、步驟 4 |
| 網路 | 防火牆來源加全組 IP | 開專案者 | 步驟 5 |
| 機密 | 換強密碼、.env 的存放與傳遞 | 全組約定 | 步驟 6 |
...
**週期建議**：把「開報表看一眼使用費」加進每週的固定動作（團體專案由開專案者負責，見〈團體專案上雲〉步驟 1）；預算警告（Part B）是被動保險，主動看報表才會知道錢流向哪裡。
