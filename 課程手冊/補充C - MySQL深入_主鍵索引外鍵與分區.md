# 補充 C：MySQL 深入 — 約束、索引、外鍵、交易與分區（含 phpMyAdmin 實戰）
...
### 容易搞混的四兄弟：PK / UNIQUE / INDEX / FK

先看家族分類——這些名詞其實來自**兩個家族**，有兩位跨界者：

```
約束家族（管「規則」）              索引家族（管「速度」）
├─ PRIMARY KEY ─────┐          ┌───── INDEX（一般索引）
├─ UNIQUE ──────────┼── 跨界 ──┤      （純加速，不擋任何東西）
├─ FOREIGN KEY      │          │
└─ NOT NULL/DEFAULT └──────────┘
```

| 名詞 | 屬於 | 管什麼 | 擋重複？ | 加速查詢？ | 一表幾個 | 課程對應 |
|------|------|--------|:-------:|:---------:|:-------:|---------|
| **PRIMARY KEY** | 約束＋自帶索引（跨界）| 每列的身分證 | ✅ | ✅ | 只能一個 | `(stock_id, date)` |
| **UNIQUE** | 約束＋自帶索引（跨界）| 這欄不准重複 | ✅ | ✅ | 可多個 | email；混搭方案的自然鍵 |
| **INDEX** | **純索引** | **只**加速查詢 | ❌ **不擋！** | ✅ | 可多個 | `idx_stock_date` |
| **FOREIGN KEY** | 純約束 | 表與表掛鉤（擋孤兒）| ❌ | —（需索引支撐）| 可多個 | orders 的 user_id |
| **NOT NULL / DEFAULT** | 純約束 | 不准空／自動補值 | ❌ | ❌ | 每欄可設 | users 約束實驗 |

三句話收斂：

- **會擋重複的只有 PK 和 UNIQUE**（PK 本質＝最嚴格的 UNIQUE：不准 NULL、一表一個）
- **INDEX 是純加速**——加了 `INDEX(stock_id, date)` 的表照樣塞得進重複資料；想擋重複，要的是 PK 或 UNIQUE
- **「複合（composite）」不是第五種東西**，是修飾詞——多個欄位合組成以上任一種
...
「pruning」是 MySQL 對這個機制的正式名稱（partition pruning，中文譯「分區剪枝」）。查詢優化器在**執行之前**，先拿 WHERE 條件跟每個分區的定義比對，把「不可能有符合資料」的分區直接從執行計畫裡剪掉——像修剪樹枝一樣，被剪掉的分區連開都不開。

以上面那條查詢為例：

```
WHERE date BETWEEN '2025-06-01' AND '2025-06-13'
→ YEAR(date) 只可能是 2025
→ 比對分區定義：p2024（<2025）不可能、p2025（<2026）可能、pmax 不可能
→ 執行計畫只留 p2025
```

這就是 EXPLAIN 的 `partitions` 欄只顯示 `p2025` 的原因——那一欄顯示的就是「剪完之後剩下的分區」。比對發生在執行前、只看分區定義，不用掃 p2024 和 pmax 的資料。

分區的查詢效益整個建立在這個機制上：**條件帶到分區鍵 → 剪枝剪得掉 → 少掃**。反過來，如果查詢條件沒帶 date（例如只用 stock_id 查），優化器無從判斷哪些分區不相關，三個分區全開——這也是「查詢模式要固定帶分區鍵，分區才值得做」的原因。

### 為什麼分區表沒有比較快？

同一個範圍查詢，兩張表各跑一次（phpMyAdmin 顯示的執行時間）：

```sql
SELECT * FROM TaiwanStockPrice      WHERE date BETWEEN '2024-06-01' AND '2026-06-13';
-- 總計 1250 筆，查詢用了 0.0006 秒
SELECT * FROM TaiwanStockPrice_part WHERE date BETWEEN '2024-06-01' AND '2026-06-13';
-- 總計 1250 筆，查詢用了 0.0033 秒
```

分區表反而比較慢。三個原因：

1. **分區有固定成本**。每個分區實體上是一張獨立的表（各自的資料檔、各自的索引樹）。查詢時 MySQL 要先判斷該開哪些分區、再逐分區執行，這個成本不隨資料量縮小。
2. **這個查詢省不到掃描量**。日期範圍橫跨 2024～2026，三個分區幾乎都要開——pruning 只能跳過「完全不相關」的分區，範圍蓋住大半張表時就沒東西可跳。而且條件只有 `date`，複合主鍵 `(stock_id, date)` 以 stock_id 開頭幫不上忙，分區內部仍是逐列掃。普通表同樣是全表掃——兩邊掃的量差不多，分區那邊還多做了開分區的工作。
3. **資料量太小**。全表才兩千筆左右，掃描本來就不到 1 毫秒，分區能省的掃描量遠小於它的固定成本。另外毫秒級的單次測量本身有浮動——同一條 SQL 連跑五次，時間就會不一樣。

一句話總結：**分區減少的是「要掃幾筆」，不是「這一次跑多快」**。資料小的時候固定成本佔上風；資料大到掃描成本主導時，分區的效益才會反映在時間上。

### 多大的資料量才值得分區？

- **千萬筆（10⁷）以上、或單表數十 GB**，而且查詢模式固定會帶分區鍵（例如永遠按日期範圍查）——這時 pruning 跳過的掃描量才顯著。
- 另一個常見動機跟查詢速度無關：**定期刪除舊資料**。`ALTER TABLE ... DROP PARTITION p2024;` 是秒級操作；沒分區的話 `DELETE ... WHERE date < ...` 要逐列刪、還會產生大量交易日誌。
- 順序上**先建對索引**：多數「查詢慢」靠索引就能解決；索引解決不了的（資料量本身太大、要整批刪除歷史）才考慮分區。
- BigQuery（第 15 章）的日期分區是同一個概念的雲端版，那邊資料量級大，分區是預設就該做的事。

### 一個提醒

**分區鍵必須包含在主鍵（或唯一鍵）裡**——我們的複合主鍵 `(stock_id, date)` 含有 date，天生適合按年分區；當初若用自增 id 當主鍵，這裡就要重新設計。
