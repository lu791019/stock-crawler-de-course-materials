...
2. **這個查詢省不到掃描量**。日期範圍橫跨 2024～2026，三個分區幾乎都要開——pruning 只能跳過「完全不相關」的分區，範圍蓋住大半張表時就沒東西可跳。而且條件只有 `date`，複合主鍵 `(stock_id, date)` 以 stock_id 開頭幫不上忙（第 1 節的最左前綴），分區內部仍是逐列掃。普通表同樣是全表掃——兩邊掃的量差不多，分區那邊還多做了開分區的工作。
