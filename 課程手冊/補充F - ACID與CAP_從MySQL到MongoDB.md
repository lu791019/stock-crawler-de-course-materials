...
CAP 選了 A 的系統，工程上遵循一套跟 ACID 刻意對仗的設計哲學，叫 **BASE**（命名是化學梗：ACID 酸、BASE 鹼）：

| 字母 | 名稱 | 意思 | 連鎖餐廳例子的對應 |
|------|------|------|------------------|
| **BA** | Basically Available 基本可用 | 系統保證能回應，但回的可能是過時資料或降級後的服務 | 斷線時照常讓客人扣點 |
| **S** | Soft state 軟狀態 | 允許中間態：即使沒有新寫入，資料也可能因背景同步而變化 | 兩家分店帳目還沒對完的期間 |
| **E** | Eventually consistent 最終一致 | 不保證「現在」一致，保證停止寫入後各副本「終將」收斂一致 | 連線恢復後對帳，兩邊點數對平 |

ACID、CAP、BASE 三者是一條線，不是三個平行概念：

```
ACID ──「交易全都要保證」──► 強一致陣營（CAP 選 C）
                │
               CAP：分散式之後，C 和 A 只能選一個（定理）
                │
BASE ──「我選可用性」────► 最終一致陣營（CAP 選 A 的工程答案）
```

也就是說：**CAP 是考題，ACID 和 BASE 是兩份不同的答卷**——交 ACID 卷的是銀行轉帳，交 BASE 卷的是社群按讚。

兩個容易誤會的點：

- BASE 的三個字母**不是三選二**（結構跟 CAP 不同），也**不是三條獨立保證**（跟 ACID 不同）——它是同一個選擇的三個面向：因為要基本可用，所以允許軟狀態，所以只能最終一致。
- 「NoSQL = BASE」是簡化說法：BASE 對應的是 CAP 選 A 的系統（上表的 Cassandra、DynamoDB）；MongoDB 預設寫入走單一主節點、一致性可調，不是每個 NoSQL 都站在 BASE 那一端（下一節分工表的「可調」指的就是這件事）。
...
CAP 是定理——被證明的限制，逼你在斷線時的一致與可用之間選邊。ACID 和 BASE 都是「選完之後的承諾」：ACID 是強一致陣營的交易保證，BASE 是選了可用性之後的設計哲學。一句話：CAP 是考題，ACID 和 BASE 是兩份不同的答卷。
