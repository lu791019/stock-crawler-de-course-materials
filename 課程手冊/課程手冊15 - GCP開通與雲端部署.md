...
> 本章對應 EP17。課堂教學順序：本章完成 GCP 開通之後，下一堂回頭做第 14 章的 BigQuery 實作——那一章需要的服務帳戶與金鑰，在本章就會發好。
>
...
3. 建立服務帳戶（Service Account）並下載 JSON 金鑰——第 14 章 BigQuery 實作的通行證
...
- 每個服務的 API 要**各自啟用**：第一次用 Compute Engine 要啟用它的 API，第 14 章第一次用 BigQuery 也要啟用它的 API
...
![官網首頁與免費試用入口](images/ch15/01-官網首頁-免費試用入口.jpg)
...
![註冊步驟1帳戶資訊](images/ch15/02-註冊步驟1-帳戶資訊與條款.jpg)
...
![新增信用卡視窗](images/ch15/03-註冊步驟2-新增信用卡視窗.jpg)
...
![開通完成的控制台歡迎頁](images/ch15/05-開通完成-控制台歡迎頁.jpg)
...
![預算與警告入口頁](images/ch15/06-預算與警告-入口頁.jpg)
...
- **JSON 金鑰是這個身分的鑰匙**：程式拿著這個檔案向 Google 證明身分。第 14 章的 `GOOGLE_APPLICATION_CREDENTIALS` 環境變數指向的就是它
- **建立時不給任何角色**：權限的原則是「需要什麼、才給什麼」（最小權限）。第 14 章要用 BigQuery 時再加對應角色
...
2. 點「**+ 建立服務帳戶**」，步驟①填三個欄位：
   ...
   ![建立服務帳戶填寫完成](images/ch15/20-建立服務帳戶-填寫完成.jpg)
3. 按「**建立並繼續**」
4. 步驟②「權限」**不選任何角色**，直接按「**完成**」（步驟③一併略過）
   ![權限步驟不選角色](images/ch15/21-建立服務帳戶-權限步驟不選角色.jpg)
...
1. 從「下載」資料夾移到固定位置，並限制讀取權限：
   ...
   ```
2. 這個路徑第 14 章會填進 `GOOGLE_APPLICATION_CREDENTIALS`，先記住放在哪
3. 永遠不放進任何 git 專案資料夾
...
![雲端 Airflow 登入頁](images/ch15/32-雲端Airflow登入頁.jpg)

登入 Airflow 之後看到的 DAG 清單，跟第 13 章在本機看到的完全相同——同一份程式碼、同一組容器，只是機器換到了雲端：

![雲端 Airflow DAGs 清單](images/ch15/33-雲端Airflow-DAGs清單.jpg)

Flower 上兩個 worker Online、各自 Succeeded——剛才發的任務在這裡留下紀錄:

![雲端 Flower](images/ch15/34-雲端Flower兩worker.jpg)
